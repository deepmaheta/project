...
if (isset($_POST['submit']) && $_POST['submit'] == "add") {
    ...
    if (move_uploaded_file($_FILES['productimage']['tmp_name'], $path)) {
        $catqry = "INSERT INTO product (productname,productimage,productprice,categoryid) VALUES ('" . $productname . "','" . $newname . "','".$productprice."','".$categoryid."')";
    ...
    if ($_FILES['productimage']['name'] == NULL) {
        extract($_POST);
        $qry = "UPDATE product SET productname='" . $productname . "' , productprice='".$productprice."',categoryid='".$categoryid."' WHERE id='" . $id . "'";
        $res = mysqli_query($conn, $qry);
        header("location:product.php");
    } else {
        extract($_POST);
        ...
        $path = '../images/product/' . $newname;
        if (move_uploaded_file($_FILES['productimage']['tmp_name'], $path)) {
            $qry =  "UPDATE product SET productname='" . $productname . "',productimage='".$newname."' , productprice='".$productprice."',categoryid='".$categoryid."' WHERE id='" . $id . "'";
            $res = mysqli_query($conn, $qry);
            header("location:product.php");
        }
    }
